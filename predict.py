...
class Predictor(BasePredictor):
    ...
    def inference(
        self, text, noise, s_prev=None, diffusion_steps=5, embedding_scale=1, alpha=0.7, pad=True
    ):
        text = text.strip()
        text = text.replace('"', '')
    ...
    def predict(
        ...
        alpha: float = Input(
            description="Only used for long text inputs or in case of reference speaker, \
            determines the timbre of the speaker. Use lower values to sample style based \
            on previous or reference speech instead of text.", ge=0, le=1, default=0.3
        ),
        beta: float = Input(
            description="Only used for long text inputs or in case of reference speaker, \
            determines the prosody of the speaker. Use lower values to sample style based \
            on previous or reference speech instead of text.", ge=0, le=1, default=0.7
        ),
        diffusion_steps: int = Input(description="Number of diffusion steps", ge=0, le=50, default=10),
        embedding_scale: float = Input(description="Embedding scale, use higher values for pronounced emotion", ge=0, le=5, default=1),
        seed: int = Input(description="Seed for reproducibility", default=0)
    ) -> Path:
        ...
        end_time = time.time()
        print("Style computation took", str(int(end_time - start_time)), "seconds")

        wavs = []
        s_prev = None
        sentences = split_and_recombine_text(text, 370, 420)
